...
exports.processReceiptImage = void 0;
...
exports.processReceiptImage = functions.https.onCall(async (data, context) => {
    ...
    if (!geminiApiKey) {
        throw new functions.https.HttpsError('failed-precondition', 'Gemini API key not configured. Add GEMINI_API_KEY to functions/.env');
    }
...
- amount: the TOTAL/NET AMOUNT DUE as a plain number only`;
        ...
        if (jsonMatch) {
            ...
            if (parsed.title)
                extractedTitle = String(parsed.title);
            const num = parseFloat(String((_a = parsed.amount) !== null && _a !== void 0 ? _a : '0'));
            if (!isNaN(num) && num > 0)
                extractedAmount = num;
        }
        ...
        console.error('[Backend] Gemini error:', aiErr === null || aiErr === void 0 ? void 0 : aiErr.message);
    ...
    const splitAmong = allMemberUids && allMemberUids.length > 0 ? allMemberUids : [uploaderUid];
